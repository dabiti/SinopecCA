...
<html>
	<head>
		...
		$(function(){
			...
			function madeYinjkh(){
				var yinjkh=$("#valiyinjkh").val();
				if(yinjkh!=null&&yinjkh.length==8){
					var shh='${clerk.parentorg}';
					$("#valiyinjkh").val(shh+'00000000'+yinjkh);
		...
		//	alert(id);
			...
			if($("#"+id).val().length!=0&&$("#"+id).val()!=''){
				if(id=="valiyinjkh"){
					...
					if($("#valiyinjkh").val()!=yinjkh){
						$("#"+id+"msg").text("* "+name+"未修改,请到资料修改中进行变更");
						return false;
						}
				}
					...
					if (textStatus = "success") {
						...
						if("fail001" == data){
							...
							if(accountinfo[14]=="销户"){
								flag =  false; 
								$("#zhanghmsg").text("该账户状态不合法，无法进行建库或变更");
								return;
							}
							if(accountinfo[10]=="1"){
								flag =  false; 
								$("#zhanghmsg").text("该账户共用其他账户印鉴，无法进行建库或变更");
								return;
							}
							if(accountinfo[17]=="1"){
								$("#managetype").val('2');
							...
							//印鉴卡编号
							$("#yinjkbh").val(accountinfo[9]);
							if($("#managetype").val()=='0'){
								$("#valiyinjkh").val(accountinfo[9]);
								$("#valiyinjkh").attr("readonly","readonly").css("color","silver");
							}
		...
		function verifyAll(){
				...
				if(cjlx!=6){
					if(requestForZhangh()&&volidate(0,'qiyrq','启用日期')&&volidate(0,'yinjtx','印鉴图像')&&volidate(5,'valiyinjkh','印鉴卡编号')){
						return true;
					}
				}else{
					if(requestForZhangh()&&volidate(0,'qiyrq','启用日期')&&volidate(5,'valiyinjkh','印鉴卡编号')){
						return true;
					}
				}
				return false;
				...
				<tr>
					...
					<td>
						...
						<tr>
								...
								<td width="55%" class="class1_td alignleft">
								<input name="valiyinjkh" id="valiyinjkh" 
										class="inputField required zhangh" maxlength="22" onkeydown=" if(event.keyCode==13){if(!volidate(5,'valiyinjkh','印鉴卡编号')){this.focus();return false;}}if(event.keyCode==9){if(!volidate(5,'valiyinjkh','印鉴卡编号')){this.focus();return false;}}" 
										 onblur="volidate(5,'valiyinjkh','印鉴卡编号')" style="width: 160px;"/>
									
									<span id="valiyinjkhmsg" style="color: red"></span> 
									<input type="hidden"  id="yinjkbh" name="yinjkbh">
								</td>
							</tr>
		 				<tr>
