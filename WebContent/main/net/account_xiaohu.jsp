...
function getAccountInfo() {
	...
	$.post("accountinfo.do?method=getAccountForNet", {math:math, account:account}, function (data, textStatus) {
		if (textStatus = "success") {
			...
			if(accountinfo[4]==1){
				if(accountinfo[9]!=null&&accountinfo[9].length!=0){
					$("#yinjgs").show();
				var zzhlist=accountinfo[9].split("|");
				var select =$("#gszh");
					//$("#gszh option").remove();
					$("#gszh").empty();
					for(i=0;i<zzhlist.length;i++){
						var zhanghinfo=zzhlist[i].split("&");
						select.append("<option value='"+zhanghinfo[0]+"' title='开户日期:"+zhanghinfo[1]+"'>"+zhanghinfo[0]+"</option>"); 
					}
				}
			}else{
				$("#yinjgs").hide();
 ...
 //$("#zhangh").focus();
   ...
   submitHandler:function(form){
	   ...
	   if(accountstate.indexOf('销户')!=-1){alert('账户状态为：'+accountstate+',无法销户');return};
	  // if (confirm("确定要销户?"))
	 //  {	   
   <c:if  test='${tesyw_shuangrhq==1}'>
			   var obj = new Object(); 
			   obj.titleName = "销户";
			   obj.account = $("#zhangh").val();obj.checktype="授权";obj.quanxbs="XH_001";
...
<table width="100%"  border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td background="images/main_place_bg.gif">账户管理 - 账户销户</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
...
<form id="form1" name="form1"  method="post" action="accountinfo.do?method=coverAccountForNet">    
    ...
    <td>
      <table width="100%"  border="0" cellspacing="1" cellpadding="0" class="class1_table">
        <thead class="class1_thead">
          <tr>
            <th colspan="2" class="class1_thead th">账户销户</th>
          </tr>
        </thead>
		<tr>
        ...
        <tr id="yinjgs"  style="display:none">
          <td class="class1_td alignright">印鉴归属</td>
          <td class="class1_td alignleft">
          <select id="gszh" name="gszh" style="width:160px">
          </select>
          </td>
        </tr>
    </table>
...
<div class="funbutton">
  <button type="submit" style="width:60px" onmouseover="this.className='buttom2'" onmouseout="this.className='buttom1'" class="buttom2">销户</button>
  &nbsp;&nbsp;&nbsp;
  <button type="button" style="width:60px" onmouseover="this.className='buttom2'" onmouseout="this.className='buttom1'" class="buttom1" onclick="javascript:window.location.href='accountinfo.do?method=coverAccountForNetView';"><img src="images/back1.gif" width="11" height="11" align="absmiddle">取消</button>
</div>
</form>
</body>
</html>