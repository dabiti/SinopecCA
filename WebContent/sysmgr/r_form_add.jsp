...
<html>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$('#shurlx').unbind().bind('change', function() {
						  ...
						  if($('#shurlx').val()=='动态多选框'){
							  $('#shurlxList').html("数据源:<textarea  name ='shujy'  cols='30' rows='3'/><div>注意:数据源数据集合必须是两列，分别为:'key'(库值),'name'(转义)</div>");
						  }
						  if($('#shurlx').val()=='回显输入'||$('#shurlx').val()=='子集输入'){
							  $('#shurlxList').html("数据源:<textarea  name ='shujy'  cols='30' rows='3'/>");
						  }
						});
			})
		var i=0;
	...
	<body onkeydown='if(event.keyCode==78&&event.ctrlKey)return false;'>
		...
		<html:form styleId="form" action="r_formMgr.do?method=addForm&baobbs=${baobbs }"
			method="post">
					...
					<td>
						<table width="100%" border="0" cellspacing="1" cellpadding="0"
							...
							<tr>
								...
								<td class="class1_td alignleft">
									<html:text property="map(YAOSBS)"  styleId="yaosbs"  styleClass="inputField required BAOBBS" />
									*组合主键，jsp元素ID，尽量按规范命名,如：clerknum
								</td>
								<td class="class1_td alignleft"></td>
							</tr>
							<tr>
								...
								<td class="class1_td alignleft">
									<html:text property="map(YAOSMC)" styleId='zdmc'   styleClass="inputField required" onfocus="javaScript:autoCompleteForTableLine('zdmc',{tableName:this.form.SHUJHQFS.value});" />
							...
							<tr>
								...
								<td class="class1_td alignleft">
									<html:text property="map(XIANSSX) " styleClass="inputField required"  />
									*表单要素的排列顺序 
								</td>
