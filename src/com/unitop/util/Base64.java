...
public class Base64 {
	...
	private static int decode4to3(byte[] source, int srcOffset,
		...
		// Example: DkLE
			...
			try {
				...
				// CommonUtil.debug(""+source[srcOffset]+ ": " + ( DECODABET[
				// source[ srcOffset ] ] ) );
				// CommonUtil.debug(""+source[srcOffset+1]+ ": " + ( DECODABET[
				// source[ srcOffset + 1 ] ] ) );
				// CommonUtil.debug(""+source[srcOffset+2]+ ": " + ( DECODABET[
				// source[ srcOffset + 2 ] ] ) );
				// CommonUtil.debug(""+source[srcOffset+3]+ ": " + ( DECODABET[
				// source[ srcOffset + 3 ] ] ) );
				return -1;
			} // e nd catch
		}
	...
	public static void main(String[] args) {
		...
		String s = "没有这句就玩不转拉 ";
		byte[] b = s.getBytes();
		String t = Base64.encodeBytes(b);
		byte[] m = Base64.decode(t);
	    String ll = new String(m);
	}
}
